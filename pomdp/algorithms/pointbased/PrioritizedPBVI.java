	...
	protected double improveValueFunction( Vector vBeliefPoints ){
		...
		for( iIteration = 0 ; ( iIteration < cIterations ) && ! bDone ; iIteration++ ){	
			...
			if( bsMax != null ){
				...
				/*
				if( vBeliefPoints.size() == 128 )
					System.out.println( "Backup for " + bsMax + " with error " + round( dMaxError, 3 ) +
						" v(bs) " + round( valueAt( bsMax ), 3 ) + " max av " + bsMax.getMaxAlpha().getId() +
						" backup " + avBackup.getId() + 
						" backup*bs " + round( avBackup.dotProduct( bsMax ), 3 ) +
						" value function changes " + m_vValueFunction.getChangesCount() );
						*/
				//add( avBackup );
				bsLast = bsMax;
		...
		System.out.println( "Last delta over " + bsLast + 
