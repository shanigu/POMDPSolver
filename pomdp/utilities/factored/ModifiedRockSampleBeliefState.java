...
public class ModifiedRockSampleBeliefState extends BeliefState {
	...
	private ModifiedRockSampleBeliefState[][] m_aSuccessors;
	...
	public ModifiedRockSampleBeliefState( ModifiedRockSample mrsPOMDP ) {
		super( mrsPOMDP.getStateCount(), mrsPOMDP.getActionCount(), mrsPOMDP.getObservationCount(), -1, true, null );
		m_mrsPOMDP = mrsPOMDP;
		...
		if( m_bCacheBelifStates ){
			m_aSuccessors = new ModifiedRockSampleBeliefState[m_cActions][m_cObservations];
	...
	*/
	public BeliefState nextBeliefState( int iAction, int iObservation ){	
		ModifiedRockSampleBeliefState bsNext = null;
		m_cBeliefUpdateRequests++;
		if( m_cBeliefUpdateRequests % 10000 == 0 )
			Logger.getInstance().log( "ModifiedRockSampleBeliefState", 0, "nextBeliefState", 
					"After " + m_cBeliefUpdateRequests + " requests " + ( 1.0 * m_cRealBeliefUpdates ) / m_cBeliefUpdateRequests );
		if( m_bCacheBelifStates ){
			if( m_aCachedObservationProbabilities[iAction][iObservation] == Double.NEGATIVE_INFINITY ){
				m_cRealBeliefUpdates++;
				bsNext = new ModifiedRockSampleBeliefState( this );
				m_aCachedObservationProbabilities[iAction][iObservation] = bsNext.applyActionAndObservation( iAction, iObservation );
			...
			bsNext = new ModifiedRockSampleBeliefState( this );
			bsNext.applyActionAndObservation( iAction, iObservation );
		}
		return bsNext;
	}
