...
public class RemoteTaskExecutionThread extends TaskExecutionThread {
		...
		try{
			...
			while( !m_bKilled ){
				...
				try{
					...
					//System.out.println( "Processing task " + tOriginal.getId() + " remotely" );
					...
					//System.out.println( "Task " + tOriginal.getId() + " processed remotely" );
	...
	*/
	private void computeTime( Task t, long lTime ){
		...
		g_cTasks++;
		if( g_cTasks % 50 == 0 ){
			System.out.println( "Average times: ComputeG - " + ( g_cCounts[0] == 0 ? 0 : g_cTime[0] / g_cCounts[0] ) + 
